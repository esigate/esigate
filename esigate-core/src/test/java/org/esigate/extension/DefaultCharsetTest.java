...
public class DefaultCharsetTest extends TestCase {
	...
	private Driver createMockDriver(Properties properties, MockHttpClient connectionManager) {
		return createMockDriver(properties, connectionManager, "tested");
	}

	private Driver createMockDriver(Properties properties, MockHttpClient connectionManager, String name) {
		CookieManager cookieManager = ExtensionFactory.getExtension(properties, Parameters.COOKIE_MANAGER, null);

		HttpClientHelper httpClientHelper = new HttpClientHelper(new EventManager(), cookieManager, connectionManager, properties);
