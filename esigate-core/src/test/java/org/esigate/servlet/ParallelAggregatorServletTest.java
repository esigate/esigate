...
/**
 * Test the parallel aggregator servlet for driver selection (url mapping) for
 * both web.xml configuration (legacy) and esigate.properties.
 * 
 * @author Nicolas Richeton
 */
public class ParallelAggregatorServletTest {
	...
	public void testConfig() throws ServletException {
		ParallelAggregatorServlet servlet = new ParallelAggregatorServlet();
	 ...
	 */
	@Test
	public void testConfigEsi() throws ServletException {
		ParallelAggregatorServlet servlet = new ParallelAggregatorServlet();
		...
		servlet.init(new ServletConfig() {
			...
			public String getInitParameter(String name) {
				...
				if (name.equals("esi_max_threads"))
					return "60";
	...
	public void testConfigEsiWithError() throws ServletException {
		ParallelAggregatorServlet servlet = new ParallelAggregatorServlet();
	...
	public void testProviderSelectionWebXml() throws ServletException, IOException, HttpErrorPage {
		...
		Assert.assertEquals("provider2", servlet.getDriverSelector().selectProvider(request).getLeft()
		...
		Mockito.when(request.getScheme()).thenReturn("http");
		Assert.assertEquals("provider1", servlet.getDriverSelector().selectProvider(request).getLeft()
				.getConfiguration().getInstanceName());
